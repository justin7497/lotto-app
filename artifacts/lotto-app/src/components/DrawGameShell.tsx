...
import { RecommendFooterSaveActions } from "@/components/RecommendSaveActions";
...
export default function DrawGameShell({
  ...
  return (
    <div className={`ball-draw-page page-content space-y-0 pb-0${done ? " ball-draw-page--done" : ""}`}>
      <PageCard className="ball-draw-page__machine-card">
        ...
        <div className="ball-draw-page__stage">{children}</div>

        <div className="ball-draw-page__footer">
          ...
          <p className="ball-draw-page__note">
            랜덤 추첨으로 번호 생성 · 실제 동행복권 추첨과 무관 · 저장 시 나의 로또번호에 보관
          </p>

          {done ? (
            <section className="ball-draw-page__save" aria-label="나의 로또번호 저장">
